...
export default function FirstYearDropdown() {
  const [isOpen, setIsOpen] = useState(false);

  const components = [
    { id: 'texts', title: 'مكون النصوص', icon: <FileText size={24} />, color: 'bg-blue-50 text-blue-600' },
    { id: 'grammar', title: 'مكون الدرس اللغوي', icon: <BookOpen size={24} />, color: 'bg-emerald-50 text-emerald-600' },
    { id: 'writing', title: 'مكون التعبير والإنشاء', icon: <PenTool size={24} />, color: 'bg-amber-50 text-amber-600' },
  ];
      ...
      <div className="relative">
        ...
        <button
          ...
          className={`w-full flex items-center justify-between p-5 rounded-2xl shadow-sm border-2 transition-all duration-300 ${
            isOpen ? 'bg-emerald-600 border-emerald-600 text-white shadow-emerald-100' : 'bg-white border-gray-100 text-gray-800 hover:border-emerald-200'
          }`}
        >
          <div className="flex items-center gap-4">
            <div className={`w-12 h-12 rounded-xl flex items-center justify-center text-2xl font-bold ${isOpen ? 'bg-white/20' : 'bg-emerald-50 text-emerald-600'}`}>
              1
            </div>
            <span className="text-xl font-black">السنة الأولى إعدادي</span>
          </div>
